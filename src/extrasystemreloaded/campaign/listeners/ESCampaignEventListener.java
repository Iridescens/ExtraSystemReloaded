...
public class ESCampaignEventListener extends BaseCampaignEventListener implements EveryFrameScript {
    @Getter private static List<CampaignFleetAPI> activeFleets = new ArrayList<>();
        ...
        if (checkFleets || esCleaningInterval.intervalElapsed()) {
            ...
            for (String fmId : fmIds) {
                ...
                if(!found) {
                    ...
                    log.info(String.format("Removed ES for missing fmId [%s]", fmId));
                }
            }
        }
    ...
    private static boolean checkNearbyFleets() {
        ...
        /*for (CampaignFleetAPI fleet : CampaignUtils.getNearbyFleets(playerFleet, 100f)) {
            ...
            log.info(String.format("Added systems to fleet [%s]", fleet.getName()));
            Es_ModPlugin.applyExtraSystemsToFleet(fleet);
        }*/
        ...
        for (Iterator<CampaignFleetAPI> it = activeFleets.iterator(); it.hasNext(); ) {
            ...
            if(removedFleet) {
                if(fleet == null) {
                    log.info("Removed a null fleet");
                } else {
                    Es_ModPlugin.removeExtraSystemsFromFleet(fleet);
                    log.info(String.format("Removed systems from fleet [%s]", fleet.getName()));
                }
                it.remove();
            }
        }
